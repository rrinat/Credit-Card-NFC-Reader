...
public class CardNfcAsyncTask extends AsyncTask<Void, Void, Object>{
    ...
    private final static String NFC_A_TAG = "android.nfc.tech.IsoDep, android.nfc.tech.NfcA";
    private final static String NFC_B_TAG = "android.nfc.tech.IsoDep, android.nfc.tech.NfcB";
    ...
    private CardNfcAsyncTask(Builder b) {
        ...
        if (mTag != null) {
            ...
            try {
                if (mTag.toString().contains(NFC_A_TAG) || mTag.toString().contains(NFC_B_TAG)) {
                    execute();
                } else {
                    if (!b.mFromStart) {
